...
gulp.task('cp', function () {
  ...
  if (typeof external.src === 'undefined' && !(external.src instanceof Array) &&
    typeof external.dest === 'undefined' && !(external.dest instanceof Array)) {
    return;
  }

  if (external.src.length !== external.dest.length) {
    return;
  }

  var cleanPaths = [];
  var src = external.src;
  ...
  var bsConfig = extend(true, {}, bsCfg, {
    ...
    rewriteRules: [{
      ...
      fn: function (match) {
        ...
        var startsWith = sassCfg.dest.substr(0, 1);
